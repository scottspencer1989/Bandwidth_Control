...
class Connection(object):
    ...
    def flush(self, name):
        ...
        #
        what=str(self.what.decode('utf-8'))
        if what=='client':
            peer=self.conn.getpeername()
            addr=self.conn.getsockname()
            try:
                host=self.conn.gethostname()
                logging.info(what + ': gethostname - ' + str(host))
            except:
                logging.info('conn.gethostname did not work')
            try:
                host=socket.gethostname()
                logging.info(what + ': gethostname - ' + str(host))
            except:
                logging.info('socket.gethostname did not work')
        ...
        logging.info(what + ': peer -' + str(peer))
        logging.info(what + ': self -' + str(addr))
        with open(name,'a') as writefile:
            output=csv.writer(writefile)
            output.writerow([datetime.datetime.now().strftime("%m_%d_%Y___%H_%M_%S"), what, addr, sent])
        #self.conn.getpeername(), self.conn.getsockname()
        #
...
class Proxy(multiprocessing.Process):
    ...
    def __init__(self, client, output):
        ...
        #self.phost, self.pport = self.request.url.path.split(COLON)
        self.output=output
    ...
    def _process_rlist(self, r):
        if self.client.conn in r:
          #  logger.debug('client is ready for reads, reading')
            ...
            #
            #with open(self.output,'a') as writefile:
            #    output=csv.writer(writefile)
            #    output.writerow([datetime.datetime.now().strftime("%m_%d_%Y___%H_%M_%S"),str(self.what.decode('utf-8')), self.client.conn.addr, data])
            #
...
class TCP(object):
    ...
    def __init__(self, output, hostname='127.0.0.1', port=8899, backlog=100):
        self.output=output
...
#See if opening and closing csv in flush works