    ...
    def recv(self, bytes=8192):
        try:
            ...
            if len(data) == 0:
                ...
                return None
            logger.debug('rcvd %d bytes from %s - %s' % (len(data), self.what.decode('utf-8'), self.conn.getpeername()))
            with open(self.name,'a') as writefile:
                output=csv.writer(writefile)
                output.writerow([datetime.datetime.now().strftime("%m_%d_%Y___%H_%M_%S"), self.what.decode('utf-8'), self.conn.getpeername(),len(data)])
            return data
...
class Server(Connection):
    ...
    def __init__(self, name, host, port):
        super(Server, self).__init__(b'server', name)
...
class Client(Connection):
    ...
    def __init__(self, name, conn, addr):
        super(Client, self).__init__(b'client', name)
        self.conn = conn
        self.addr = addr
...
class Proxy(multiprocessing.Process):
    ...
    def _process_request(self, data):
        ...
        if self.request.state == HTTP_PARSER_STATE_COMPLETE:
            ...
            self.server = Server(self.output, host, port)
    ...
    def _access_log(self):
        ...
        if self.request.method == b"CONNECT":
            logger.debug("%s:%s - %s %s:%s" % (self.client.addr[0], self.client.addr[1], self.request.method, host, port))
        elif self.request.method:
            logger.debug("%s:%s - %s %s:%s%s - %s %s - %s bytes" % (self.client.addr[0], self.client.addr[1], self.request.method, host, port, self.request.build_url(), self.response.code, self.response.reason, len(self.response.raw)))
    ...
    def _process(self):
        ...
        while True: #os.patht(.exists(os.path.join(os.getcwd(),'Tokenbin/token'))):
...
class TCP(object):
    ...
    def __init__(self, output, hostname='127.0.0.1', port=8888, backlog=100):
        self.output=output
    ...
    def run(self):
        try:
            ...
            #os.path.exsists(os.path.join(os.getcwd(),Tokenbin/token)
            while True: #os.path.exists(os.path.join(os.getcwd(),'Tokenbin/token')): #Put some toggle here search for python ls
                conn, addr = self.socket.accept()
                logger.debug('Accepted connection %r at address %r' % (conn, addr))
                client = Client(self.output, conn, addr)
                self.handle(client, self.output)
        except Exception as e:
...
class HTTP(TCP):
    ...
    def handle(self, client, output):
        proc = Proxy(client, output)
        proc.daemon = True
        proc.start()
...
def main(output):
    ...
    port = int(args.port)
    try:
        proxy = HTTP(output, hostname, port)
        proxy.run()
    except KeyboardInterrupt:
        pass
...
if __name__ == '__main__':
    print("\n Okay Okay Okay! \n")
    r=random.choice(['San', 'Dimas', 'High', 'School', 'Foot', 'Ball', 'Rules'])
    name=datetime.datetime.now().strftime("%m_%d_%Y___%H_%M_%S")+r+'.csv'
